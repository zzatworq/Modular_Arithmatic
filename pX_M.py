...
for idx, x in enumerate(x_values):
    ...
    plt.text(N * 0.7, max(seen) * 0.7, rf"\textbf{{Cycle}}: $\{{ {cycle_str} \}}$", 
         fontsize=14, color=color, bbox=dict(facecolor='white', alpha=0.8, edgecolor='black'))


    # Save plot
    filename = f"imgs/{x:05d}_mod{MODULO:05d}_cyclesfor_{x}.png"
    plt.tight_layout()
